...
export function createVisionRecognitionPipeline({
  ...
  function cropImageToCanvas(source, box, options = {}) {
    const rect = getCropPixelRect(source, box, options);
    const maxDimension = Number(options.maxDimension) || Math.max(rect.width, rect.height);
    const scale = Math.min(1, maxDimension / Math.max(rect.width, rect.height));
    const canvas = document.createElement("canvas");
  ...
  const {
    ...
    nativeEmbedImageDataUrls: platform.vision?.canUseNativeImageEmbedding?.()
      ? platform.vision.embedImageDataUrls
      : null,
    nativeEmbedImageRegions: platform.vision?.canUseNativeRegionEmbedding?.()
      ? platform.vision.embedImageRegions
      : null,
    visionCatalog,
    visionConfig,
  });

  async function warmCatalogNamingResources() {
    if (visionConfig.catalogNamingEnabled === false) return { skipped: true, reason: "catalog-naming-disabled" };
    const startedAt = performance.now();
    const canWarmNative = Boolean(
      visionConfig.nativeCatalogEmbeddingEnabled
      && platform.vision?.canWarmUpNativeImageEmbedding?.()
    );
    const indexPromise = getCatalogEmbeddingIndex().catch((error) => ({ entries: [], error }));
    const [index, searchWarmup, embeddingRuntime] = await Promise.all([
      indexPromise,
      canWarmNative
        ? Promise.resolve({ skipped: true, reason: "native-index-warmup" })
        : indexPromise.then(() => preloadCatalogSearchValues()).catch((error) => ({ error })),
      canWarmNative
        ? platform.vision.warmUpImageEmbedding({
          model: visionConfig.catalogModel,
          indexPath: visionConfig.catalogIndexPackage,
        }).catch((error) => ({ error }))
        : getCatalogFeatureExtractor().catch(() => null),
    ]);
    const nativeReady = canWarmNative && embeddingRuntime?.ready === true;
    return {
      skipped: false,
      entries: index?.entries?.length || 0,
      vectorEntries: searchWarmup?.entries || 0,
      vectorDimension: searchWarmup?.dimension || 0,
      vectorReady: Boolean(searchWarmup?.loaded),
      extractorReady: canWarmNative ? nativeReady : Boolean(embeddingRuntime),
      embeddingWarmupMode: nativeReady ? (embeddingRuntime.mode || "native") : (canWarmNative ? "native-failed" : "web"),
      embeddingWarmupError: embeddingRuntime?.error?.message || embeddingRuntime?.error || "",
      warmupMs: Math.round((performance.now() - startedAt) * 1000) / 1000,
    };
  }
  ...
  function isAutoGeneratedCandidateName(name) {
    const value = String(name || "").trim();
    return !value
      || value.startsWith("候选区域")
      || value.startsWith("手动主体框")
      || isUnknownObjectName(value);
  }

  function getCatalogCandidateName(catalogMatch) {
    return catalogMatch?.catalogCandidates?.find((entry) => entry.displayName)?.displayName || "";
  }

  function getResolvedCandidateName(candidate, index, catalogMatch = null) {
    if (!isAutoGeneratedCandidateName(candidate.name)) return candidate.name;
    return getCatalogCandidateName(catalogMatch) || getUnknownObjectName(index);
  }

  function getNamingOutcome(candidate, index, catalogMatch = null) {
    if (catalogMatch?.accepted) return "accepted";
    if (catalogMatch?.catalogCandidates?.length) return "candidate-only";
    const name = getResolvedCandidateName(candidate, index, catalogMatch);
    return isAutoGeneratedCandidateName(name) || isUnknownObjectName(name) ? "generic-fallback" : "candidate-only";
  }

  function createNamingDiagnostics(candidate, index, catalogMatch, outcome, extra = {}) {
    const candidates = Array.isArray(catalogMatch?.catalogCandidates) ? catalogMatch.catalogCandidates : [];
    const topK = Array.isArray(catalogMatch?.catalogTopK) ? catalogMatch.catalogTopK : candidates;
    const best = candidates[0] || topK[0] || null;
    const policy = catalogMatch?.namingAcceptancePolicy || null;
    return {
      subjectId: candidate.id || "",
      sourceImageId: extra.sourceImageId || candidate.sourceImageId || "",
      ...
      modelBox: extra.modelBox || candidate.modelBox || null,
      cropMeta: normalizeCropMeta(candidate.cropMeta),
      ...
      margin: Number(catalogMatch?.categoryMargin ?? 0) || 0,
      acceptancePolicy: policy,
      rejectionReason: catalogMatch?.namingRejectionReason || "",
      unnameableReason: extra.unnameableReason || "",
      matchedSampleIds: catalogMatch?.matchedSampleIds || best?.matchedSampleIds || [],
      timings: {
        ...(candidate.timings || {}),
  ...
  function candidateBoxArea(candidate) {
    const box = candidate?.box || {};
    return Math.max(0, Number(box.w) || 0) * Math.max(0, Number(box.h) || 0);
  }

  function getNamingPriority(candidate, index) {
    return (candidate.selected ? 0.2 : 0)
      + clampNumber(candidate.confidence || 0, 0, 1)
      + Math.min(1.2, candidateBoxArea(candidate) / 420)
      - (index * 0.0001);
  }

  function resolveCandidateFromCatalogMatch(candidate, index, catalogMatch) {
    if (catalogMatch?.accepted) {
      const namingOutcome = "accepted";
      return {
        ...candidate,
        ...
        catalogTopK: catalogMatch.catalogTopK || catalogMatch.catalogCandidates || [],
        namingRejectionReason: "",
        namingOutcome,
        unnameableReason: "",
        namingAcceptancePolicy: catalogMatch.namingAcceptancePolicy || null,
        namingDiagnostics: createNamingDiagnostics(candidate, index, catalogMatch, namingOutcome),
    ...
    if (catalogMatch) {
      const bestCandidate = catalogMatch.catalogCandidates?.[0] || null;
      const namingOutcome = getNamingOutcome(candidate, index, catalogMatch);
      const unnameableReason = catalogMatch.namingRejectionReason || (bestCandidate ? "" : "no-catalog-candidate");
      return {
        ...candidate,
        name: getResolvedCandidateName(candidate, index, catalogMatch),
        category: bestCandidate?.appCategory || candidate.category,
        ...
        unnameableReason,
        namingAcceptancePolicy: catalogMatch.namingAcceptancePolicy || null,
        namingDiagnostics: createNamingDiagnostics(candidate, index, catalogMatch, namingOutcome, { unnameableReason }),
        ocrText: catalogMatch.ocrText || "",
        categoryIndexVersion: catalogMatch.categoryIndexVersion || "",
        matchedSampleIds: catalogMatch.matchedSampleIds || [],
        timings: {
  ...
  async function resolveCandidateName(candidate, index, source, options = {}) {
    if (candidate.nameEdited && !options.force) return { ...candidate, namingStatus: "done" };
    if (visionConfig.catalogNamingEnabled === false) {
      const namingOutcome = "generic-fallback";
      return {
        ...candidate,
        name: getResolvedCandidateName(candidate, index),
        namingStatus: "done",
        namingRejectionReason: "catalog-naming-disabled",
        namingOutcome,
        unnameableReason: "catalog-naming-disabled",
        namingDiagnostics: createNamingDiagnostics(candidate, index, null, namingOutcome, {
          unnameableReason: "catalog-naming-disabled",
        }),
      };
    }
    ...
    if (embeddingIndex.entries?.length) {
      ...
      const catalogMatch = source ? await matchCatalogFromEmbeddingIndex(source, embeddingBox, {
        ...
        confidence: 0,
        catalogCandidates: [],
        namingRejectionReason: `catalog-error:${error?.message || "unknown"}`,
      })) : null;
      const resolved = resolveCandidateFromCatalogMatch(candidate, index, catalogMatch);
    ...
    return {
      ...
      name: getResolvedCandidateName(candidate, index),
      namingStatus: "done",
      namingOutcome: "generic-fallback",
      unnameableReason: "catalog-index-empty",
      namingDiagnostics: createNamingDiagnostics(candidate, index, null, "generic-fallback", {
        unnameableReason: "catalog-index-empty",
      }),
    };
  }
  ...
  async function nameDetectedCandidates(input, candidates, onProgress) {
    ...
    const modelImageDataUrl = typeof modelImage === "string" ? modelImage : "";
    const modelContext = context.modelContext || null;
    const minimumAnimation = Promise.resolve();
    ...
    const preparedCandidates = displaySource
      ? candidates.map((candidate) => (
        shouldRefreshCandidateCrop(candidate)
          ? { ...candidate, ...(createCandidateCropSnapshot(displaySource, candidate.box) || {}), namingStatus: "loading" }
          : { ...candidate, namingStatus: "loading" }
      ))
      : candidates.map((candidate) => ({ ...candidate, namingStatus: "loading" }));
    const named = Array(preparedCandidates.length).fill(null);
    const concurrency = Math.max(1, Math.round(Number(visionConfig.catalogNamingConcurrency) || 5));
    onProgress?.(preparedCandidates);

    const nameCandidateAtIndex = async (index) => {
      ...
      const sourceImageId = context.sourceImageId || context.imageId || "";
      ...
      named[index] = {
        ...
        namingDiagnostics: {
          ...(resolved.namingDiagnostics || {}),
          sourceImageId,
          modelBox,
          timings: {
            ...(resolved.namingDiagnostics?.timings || {}),
            namingMs: candidateNamingMs,
          },
        },
      };
    ...
    const nameCandidatesIndividually = async () => {
      let nextIndex = 0;
      const runWorker = async () => {
        while (nextIndex < preparedCandidates.length) {
          const index = nextIndex;
          nextIndex += 1;
          await nameCandidateAtIndex(index);
        }
      };
      await Promise.all(Array.from(
        { length: Math.min(concurrency, preparedCandidates.length) },
        () => runWorker(),
      ));
    };

    const nameCandidatesWithEmbeddingBatches = async () => {
      const source = modelSource || displaySource;
      const maxBatchSize = Math.max(1, Math.round(Number(visionConfig.catalogEmbeddingBatchSize) || 1));
      if (!source || visionConfig.catalogNamingEnabled === false || maxBatchSize <= 1) return false;
      const embeddingIndex = await getCatalogEmbeddingIndex();
      if (!embeddingIndex.entries?.length) return false;
      const pendingIndexes = preparedCandidates
        .map((candidate, index) => ({ candidate, index }))
        .filter(({ candidate }) => !candidate.nameEdited)
        .sort((left, right) => getNamingPriority(right.candidate, right.index) - getNamingPriority(left.candidate, left.index));
      if (!pendingIndexes.length) return false;

      const runEmbeddingBatches = async (targetIndexes) => {
        let batchSize = maxBatchSize;
        let offset = 0;
        while (offset < targetIndexes.length) {
        const batch = targetIndexes.slice(offset, offset + batchSize);
        const batchStartedAt = performance.now();
        const boxes = batch.map(({ candidate }) => (
          candidate.modelBox || (modelContext ? mapDisplayBoxToModelBox(candidate.box, modelContext) : candidate.box)
        ));
        const optionsList = batch.map(({ candidate }) => ({
          name: candidate.name,
          suggestedName: candidate.suggestedName,
          detectionLabel: candidate.detectionLabel,
          roomType: getCapturePromptRoomType?.(),
        }));
        let matches = null;
        try {
          matches = await matchCatalogBatchFromEmbeddingIndex(source, boxes, optionsList, {
            sourceImage: modelImageDataUrl,
          });
        } catch (error) {
          if (batchSize > 2) {
            batchSize -= 1;
            console.info(`Catalog embedding batch size reduced to ${batchSize}.`, error);
            continue;
          }
          throw error;
        }
        const batchNamingMs = Math.round((performance.now() - batchStartedAt) * 1000) / 1000;
        batch.forEach(({ candidate, index }, batchIndex) => {
          const resolved = resolveCandidateFromCatalogMatch(candidate, index, matches[batchIndex]) || {
            ...candidate,
            name: getResolvedCandidateName(candidate, index),
            namingStatus: "done",
            namingOutcome: "generic-fallback",
            unnameableReason: "catalog-match-empty",
          };
          named[index] = {
            ...resolved,
            timings: {
              ...(candidate.timings || {}),
              ...(resolved.timings || {}),
              namingMs: roundNumber(batchNamingMs / Math.max(1, batch.length), 3),
              namingBatchSize: batch.length,
              namingBatchMs: batchNamingMs,
            },
            namingDiagnostics: {
              ...(resolved.namingDiagnostics || {}),
              sourceImageId: context.sourceImageId || context.imageId || "",
              modelBox: boxes[batchIndex],
              timings: {
                ...(resolved.namingDiagnostics?.timings || {}),
                namingMs: roundNumber(batchNamingMs / Math.max(1, batch.length), 3),
                namingBatchSize: batch.length,
      ...
      const foregroundLimit = Math.max(1, Math.round(Number(visionConfig.catalogEmbeddingForegroundCount) || pendingIndexes.length));
      const foregroundIndexes = pendingIndexes.slice(0, foregroundLimit);
      const backgroundIndexes = pendingIndexes.slice(foregroundLimit);
      await runEmbeddingBatches(foregroundIndexes);

      if (backgroundIndexes.length) {
        const runBackground = () => {
          runEmbeddingBatches(backgroundIndexes)
            .then(() => {
              onProgress?.(preparedCandidates.map((entry, entryIndex) => named[entryIndex] || entry));
            })
            .catch((error) => {
              console.info("Catalog background naming failed.", error);
              backgroundIndexes.forEach(({ candidate, index }) => {
                if (named[index]) return;
                named[index] = {
                  ...candidate,
                  namingStatus: "done",
                  namingRejectionReason: "background-naming-failed",
                };
              });
              onProgress?.(preparedCandidates.map((entry, entryIndex) => named[entryIndex] || entry));
            });
    ...
    const usedBatchEmbedding = await nameCandidatesWithEmbeddingBatches().catch((error) => {
      console.info("Catalog batch naming failed, falling back to individual naming.", error);
      return false;
    });
    if (!usedBatchEmbedding) {
      await nameCandidatesIndividually();
    }
